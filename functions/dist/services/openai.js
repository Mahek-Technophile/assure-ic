...
exports.analyzeRiskWithOpenAI = analyzeRiskWithOpenAI;
...
async function getApiKeyFromKeyVault(vaultName, secretName) {
    const url = `https://${vaultName}.vault.azure.net`;
    const client = new keyvault_secrets_1.SecretClient(url, credential);
    const secret = await client.getSecret(secretName);
    return secret.value;
}
async function analyzeRiskWithOpenAI(flattenedFields) {
    if (!endpoint || !deployment)
        throw new Error("OPENAI_ENDPOINT or OPENAI_DEPLOYMENT not configured");
    ...
    if (process.env.OPENAI_API_KEY) {
        ...
        if (!token)
            throw new Error("Failed to acquire AAD token for OpenAI");
